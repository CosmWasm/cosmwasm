...
pub fn do_init<T: Display + From<Error>>(
    init_fn: &dyn Fn(&mut ExternalStorage, &ExternalApi, Params, Vec<u8>) -> Result<Response, T>,
...
pub fn do_handle<T: Display + From<Error>>(
    handle_fn: &dyn Fn(&mut ExternalStorage, &ExternalApi, Params, Vec<u8>) -> Result<Response, T>,
    params_ptr: *mut c_void,
    msg_ptr: *mut c_void,
) -> *mut c_void {
...
pub fn do_query<T: Display + From<Error>>(
    query_fn: &dyn Fn(&ExternalStorage, &ExternalApi, Vec<u8>) -> Result<QueryResponse, T>,
    msg_ptr: *mut c_void,
) -> *mut c_void {
...
fn _do_init<T: Display + From<Error>>(
    ...
    let precompiles = ExternalApi::new();
