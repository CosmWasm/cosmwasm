use cosmwasm_std::{
    entry_point, from_json_slice, to_json_binary, to_json_vec, Binary, Deps, DepsMut, Empty, Env,
...
mod tests {
    ...
    use cosmwasm_std::{coins, from_json_binary, OwnedDeps};
    ...
    fn push_and_pop() {
        ...
        let state: Item = from_json_slice(data.as_slice()).unwrap();
    ...
    fn query_list() {
        ...
        let ids: ListResponse =
            from_json_binary(&query(deps.as_ref(), mock_env(), query_msg).unwrap()).unwrap();
