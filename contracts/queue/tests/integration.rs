...
use cosmwasm_std::{from_json_binary, MessageInfo, Response};
use cosmwasm_vm::{
    testing::{
...
fn push_and_pop() {
    ...
    let item: Item = from_json_binary(&data).unwrap();
...
fn push_and_reduce() {
    ...
    let counters = from_json_binary::<ReducerResponse>(&data).unwrap().counters;
...
fn query_list() {
    ...
    let ids: ListResponse =
        from_json_binary(&query(&mut deps, mock_env(), query_msg).unwrap()).unwrap();
