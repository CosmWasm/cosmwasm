...
use cosmwasm_std::{
    coin, coins, from_json_binary, BankMsg, BankQuery, Binary, Coin, ContractResult, Event,
...
fn proper_initialization() {
    ...
    let value: OwnerResponse = from_json_binary(&res).unwrap();
...
fn supply_query() {
    ...
    let res: ChainResponse = from_json_binary(&res).unwrap();
    let res: SupplyResponse = from_json_binary(&res.data).unwrap();
...
fn dispatch_custom_query() {
    ...
    .unwrap();
    let value: CapitalizedResponse = from_json_binary(&res).unwrap();
