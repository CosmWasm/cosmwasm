...
fn proper_init_and_query() {
    ...
    assert_eq!(
        state,
        State {
            verifier: store.api().canonical_address("foo").unwrap(),
            beneficiary: store.api().canonical_address("bar").unwrap(),
            funder: store.api().canonical_address("creator").unwrap(),
        }
    );
}
...
fn proper_handle() {
    ...
    assert_eq!(
        msg,
        &CosmosMsg::Send {
            from_address: store.api().canonical_address("cosmos2contract").unwrap(),
            to_address: store.api().canonical_address("benefits").unwrap(),
...
fn failed_handle() {
    ...
    // beneficiary can release it
    let handle_params = mock_params(store.api(), "benefits", &[], &coin("1000", "earth"));
