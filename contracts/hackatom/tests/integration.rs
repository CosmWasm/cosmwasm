...
fn proper_initialization() {
    ...
    let expected_state = State {
        verifier: deps.api().canonical_address(&verifier).0.unwrap().into(),
        beneficiary: deps.api().canonical_address(&beneficiary).0.unwrap().into(),
        funder: deps.api().canonical_address(&creator).0.unwrap().into(),
    };
