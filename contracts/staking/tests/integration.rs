...
use cosmwasm_std::{coin, from_json_binary, ContractResult, Decimal, Response, Uint128, Validator};
...
fn proper_initialization() {
    ...
    let token: TokenInfoResponse = from_json_binary(&res).unwrap();
    ...
    let invest: InvestmentResponse = from_json_binary(&res).unwrap();
