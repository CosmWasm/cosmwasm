...
mod tests {
    ...
    fn signed_decimal_256_from_str_errors_for_broken_whole_part() {
        match SignedDecimal256::from_str("").unwrap_err() {
    ...
    fn signed_decimal_256_from_str_errors_for_broken_fractional_part() {
        ...
        match SignedDecimal256::from_str("1.e").unwrap_err() {
            StdError::GenericErr { msg, .. } => assert_eq!(msg, "Error parsing fractional"),
            e => panic!("Unexpected error: {e:?}"),
        }

        match SignedDecimal256::from_str("1.2e3").unwrap_err() {
            StdError::GenericErr { msg, .. } => assert_eq!(msg, "Error parsing fractional"),
            e => panic!("Unexpected error: {e:?}"),
        }

        match SignedDecimal256::from_str("1.-2").unwrap_err() {
            StdError::GenericErr { msg, .. } => assert_eq!(msg, "Error parsing fractional"),
            e => panic!("Unexpected error: {e:?}"),
        }
    }
    ...
    fn signed_decimal_256_from_str_errors_for_more_than_18_fractional_digits() {
        match SignedDecimal256::from_str("7.1234567890123456789").unwrap_err() {
        ...
        match SignedDecimal256::from_str("7.1230000000000000000").unwrap_err() {
            StdError::GenericErr { msg, .. } => {
                assert_eq!(msg, "Cannot parse more than 18 fractional digits")
            }
            e => panic!("Unexpected error: {e:?}"),
    ...
    fn signed_decimal_256_from_str_errors_for_invalid_number_of_dots() {
        match SignedDecimal256::from_str("1.2.3").unwrap_err() {
            StdError::GenericErr { msg, .. } => assert_eq!(msg, "Unexpected number of dots"),
            e => panic!("Unexpected error: {e:?}"),
        }

        match SignedDecimal256::from_str("1.2.3.4").unwrap_err() {
            StdError::GenericErr { msg, .. } => assert_eq!(msg, "Unexpected number of dots"),
            e => panic!("Unexpected error: {e:?}"),
        }
    }
    ...
    fn signed_decimal_256_from_str_errors_for_more_than_max_value() {
        // Integer
        match SignedDecimal256::from_str(
    ...
    fn signed_decimal_256_serialize() {
        assert_eq!(to_vec(&SignedDecimal256::zero()).unwrap(), br#""0""#);
        assert_eq!(to_vec(&SignedDecimal256::one()).unwrap(), br#""1""#);
        assert_eq!(to_vec(&SignedDecimal256::percent(8)).unwrap(), br#""0.08""#);
        assert_eq!(
            to_vec(&SignedDecimal256::percent(87)).unwrap(),
            br#""0.87""#
        );
        assert_eq!(
            to_vec(&SignedDecimal256::percent(876)).unwrap(),
            br#""8.76""#
        );
        assert_eq!(
            to_vec(&SignedDecimal256::percent(8765)).unwrap(),
            br#""87.65""#
        );
        assert_eq!(
            to_vec(&SignedDecimal256::percent(-87654)).unwrap(),
            br#""-876.54""#
        );
        assert_eq!(
            to_vec(&SignedDecimal256::negative_one()).unwrap(),
            br#""-1""#
        );
        assert_eq!(
            to_vec(&-SignedDecimal256::percent(8)).unwrap(),
            br#""-0.08""#
        );
    }
    ...
    fn signed_decimal_256_deserialize() {
        assert_eq!(
            from_slice::<SignedDecimal256>(br#""0""#).unwrap(),
            SignedDecimal256::zero()
        );
        assert_eq!(
            from_slice::<SignedDecimal256>(br#""1""#).unwrap(),
            SignedDecimal256::one()
        );
        assert_eq!(
            from_slice::<SignedDecimal256>(br#""000""#).unwrap(),
            SignedDecimal256::zero()
        );
        assert_eq!(
            from_slice::<SignedDecimal256>(br#""001""#).unwrap(),
            SignedDecimal256::one()
        );

        assert_eq!(
            from_slice::<SignedDecimal256>(br#""0.08""#).unwrap(),
            SignedDecimal256::percent(8)
        );
        assert_eq!(
            from_slice::<SignedDecimal256>(br#""0.87""#).unwrap(),
            SignedDecimal256::percent(87)
        );
        assert_eq!(
            from_slice::<SignedDecimal256>(br#""8.76""#).unwrap(),
            SignedDecimal256::percent(876)
        );
        assert_eq!(
            from_slice::<SignedDecimal256>(br#""87.65""#).unwrap(),
