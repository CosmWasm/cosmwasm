...
mod tests {
    ...
    fn contract_info_response_serialization() {
        let response = ContractInfoResponse {
            ...
            ibcv2_port: Some("wasm.123".to_string()),
        };
        let json = to_json_binary(&response).unwrap();
        assert_eq!(
            String::from_utf8_lossy(&json),
            r#"{"code_id":67,"creator":"jane","admin":"king","pinned":true,"ibc_port":"wasm.123","ibcv2_port":"wasm.123"}"#,
        );
    }
