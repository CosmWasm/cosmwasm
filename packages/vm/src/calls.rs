    ...
    Q: Querier + 'static,
{
    ...
    let result: ContractResult<Response> = from_slice(&data, deserialization_limits::RESULT_SUDO)?;
    Ok(result)
}
    ...
    Q: Querier + 'static,
{
    ...
    let result: ContractResult<Response> = from_slice(&data, deserialization_limits::RESULT_REPLY)?;
    Ok(result)
}
...
#[cfg(feature = "ibc2")]
pub fn call_ibc2_packet_ack<A, S, Q>(
    instance: &mut Instance<A, S, Q>,
    env: &Env,
...
#[cfg(feature = "ibc2")]
pub fn call_ibc2_packet_send<A, S, Q>(
    instance: &mut Instance<A, S, Q>,
    env: &Env,
...
mod tests {
    ...
    fn call_instantiate_handles_missing_export() {
        ...
        let err = call_instantiate::<_, _, _>(&mut deps, &mock_env(), &info, &serialized_msg)
            .unwrap_err();
    ...
    fn call_execute_runs_out_of_gas() {
        ...
        let msg = br#"{"cpu_loop":{}}"#;
        let err = call_execute::<_, _, _>(&mut instance, &mock_env(), &info, msg).unwrap_err();
    ...
    mod ibc {
        ...
        fn setup(
            ...
            let res: IbcBasicResponse =
                call_ibc_channel_connect::<_, _, _>(instance, &mock_env(), &handshake_connect)
                    .unwrap()
                    .unwrap();
    ...
    mod ibc2 {
        ...
        fn call_ibc2_packet_ack_works() {
            ...
            call_ibc2_packet_ack::<_, _, _>(&mut instance, &mock_env(), &ibc2_ack)
                .unwrap()
                .unwrap();
        }
        ...
        fn call_ibc2_packet_send_works() {
            ...
            call_ibc2_packet_send::<_, _, _>(&mut instance, &mock_env(), &ibc2_sent)
                .unwrap()
                .unwrap();
        }
    }
