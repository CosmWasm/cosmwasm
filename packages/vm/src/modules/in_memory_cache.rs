...
mod tests {
    ...
    fn len_works() {
        ...
        // Add 1
        let (module_1, _) = compile_module(&wasm1, TESTING_MEMORY_LIMIT).unwrap();
        let module = CachedModule {
            module: module_1,
            engine: make_runtime_engine(TESTING_MEMORY_LIMIT),
        ...
        // Add 2
        let (module_2, _) = compile_module(&wasm2, TESTING_MEMORY_LIMIT).unwrap();
        let module = CachedModule {
            module: module_2,
            engine: make_runtime_engine(TESTING_MEMORY_LIMIT),
            size_estimate: 900_000,
        ...
        let (module_3, _) = compile_module(&wasm3, TESTING_MEMORY_LIMIT).unwrap();
        let module = CachedModule {
            module: module_3,
            engine: make_runtime_engine(TESTING_MEMORY_LIMIT),
    ...
    fn size_works() {
        ...
        // Add 1
        let (module_1, _) = compile_module(&wasm1, TESTING_MEMORY_LIMIT).unwrap();
        let cached_module_1 = CachedModule {
            module: module_1,
            engine: make_runtime_engine(TESTING_MEMORY_LIMIT),
            size_estimate: 900_000,
        };
        cache.store(&checksum1, cached_module_1).unwrap();
        assert_eq!(cache.size(), 900_032);

        // Add 2
        let (module_2, _) = compile_module(&wasm2, TESTING_MEMORY_LIMIT).unwrap();
        let cached_module_2 = CachedModule {
            module: module_2,
            engine: make_runtime_engine(TESTING_MEMORY_LIMIT),
            size_estimate: 800_000,
        };
        cache.store(&checksum2, cached_module_2).unwrap();
        assert_eq!(cache.size(), 900_032 + 800_032);

        // Add 3 (pushes out the previous two)
        let (module_3, _) = compile_module(&wasm3, TESTING_MEMORY_LIMIT).unwrap();
        let cached_module_3 = CachedModule {
            module: module_3,
            engine: make_runtime_engine(TESTING_MEMORY_LIMIT),
            size_estimate: 1_500_000,
        };
        cache.store(&checksum3, cached_module_3).unwrap();
