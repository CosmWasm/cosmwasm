...
mod test {
    ...
    fn init_cached_contract() {
        ...
        // call and check
        let res = call_init::<_, _, _, RawMsg>(&mut instance, &env, msg).unwrap();
    ...
    fn run_cached_contract() {
        ...
        let msg = br#"{"release":{}}"#;
        let res = call_handle::<_, _, _, RawMsg>(&mut instance, &env, msg).unwrap();
        let msgs = res.unwrap().messages;
    ...
    fn resets_gas_when_reusing_instance() {
        ...
        let msg = r#"{"verifier": "sue", "beneficiary": "mary"}"#.as_bytes();
        call_init::<_, _, _, RawMsg>(&mut instance1, &env, msg)
            .unwrap()
            .unwrap();
    ...
    fn recovers_from_out_of_gas() {
        ...
        match call_init::<_, _, _, RawMsg>(&mut instance1, &env1, msg1) {
        ...
        call_init::<_, _, _, RawMsg>(&mut instance2, &env2, msg2)
            .unwrap()
            .unwrap();
    }
}
