    ...
    W: io::Write,
{
    ...
    schema.definitions.iter().try_for_each(|node| {
        ...
        match language {
            Language::Rust => {
                cw_schema_codegen::rust::process_node(output, schema, node, add_heading_stuff)
            }
            Language::Typescript => {
                cw_schema_codegen::typescript::process_node(output, schema, node, add_heading_stuff)
            }
            Language::Python => cw_schema_codegen::python::process_node(output, schema, node),
            Language::Go => {
                cw_schema_codegen::go::process_node(output, schema, node, add_heading_stuff)
            }
        }
    })?;
